...
class ImageDataset(Dataset):
    def __init__(self, image_paths, true_labels, processor):
    ...
    def __getitem__(self, idx):
        ...
        try:
            with Image.open(img_path).convert("RGB") as img:
                ...
                return inputs["pixel_values"].squeeze(0), label, img_path
...
def main():
    # 参数
    dataset_path = r"C:\Users\chy\Desktop\dataset Chinese"
    positive_classes = ["狗", "猫", "瓷器", "马", "水墨画"]
    negative_classes = ["狼", "猞猁", "陶器", "驴", "油画"]
    batch_size = 32
    ...
    for cls in positive_classes:
        text_desc = f"一张{cls}的图片"
        inputs = text_tokenizer([text_desc], return_tensors="pt", padding=True).to(device)
        with torch.no_grad():
    ...
    dataset = ImageDataset(file_paths, true_labels, processor)
    dataloader = DataLoader(dataset, batch_size=batch_size, shuffle=False, num_workers=4, collate_fn=collate_fn, pin_memory=True)
